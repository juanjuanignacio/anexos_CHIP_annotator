...
def read_vcf_file(filename, outputname):
    ...
    with open(filename) as f:
        ...
        with open(outputname, 'w') as g:
            g.write('[')
        with open('variant_per_sample_short_json_for_pandas.json', 'w') as g:
            g.write('[')
        ...
        with open(outputname, 'a') as g:
            g.write(']')
        with open('variant_per_sample_short_json_for_pandas.json', 'a') as g:
            g.write(']')
...
def serialize_allAnnotations(data, canonical_transcripts, artifacts, internally_identified, whitelist_ncl, whitelist_aa, previously_identified):
    ...
    try:
        all_annotations['IMPACT'] = data['annotation']['IMPACT']
    except:
        try:
            all_annotations['IMPACT'] = data['annotation']['CSQ']['IMPACT']
        except:
            all_annotations['IMPACT'] = 'NA'
    """
...
def seems_somatic(anotation):
    ...
    def is_in_population():
        ...
        max_af = anotation.get('MAX_AF', 'NO')
        if max_af == 'NO':
...
def first_filter(ej):
    ...
    if list_filter == [] and i != 2:
        if ej['_allAnnotations']['MAX_VAF'] != 'NA': #si max_vaf es nulo es que no hay samples
            ej['_allAnnotations']['FILTER_1'] = 1
        return ej
...
def json_for_pandas(data):
    ...
    if data['_allAnnotations']['FILTER_2'] == 1:
        for sample in data['caseLevelData']:
            variant['sample'] = sample['biosampleId']
            variant['AF'] = sample['AF']
            for item in data['_allAnnotations']:
                variant[item] = data['_allAnnotations'][item]


            save_data_as_json(variant, 'variant_per_sample_short_json_for_pandas.json')
